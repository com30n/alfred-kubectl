...
def get_nodes():
    ...
    for node in nodes.items:
        ...
        subtitle = "Active: {}, Version: {}".format(
            node.status.conditions[node_active_index].status,
...
def get_pvs():
    ...
    for pv in pvs.items:
        ...
        subtitle = "Namespace: {}, Ref: {}, Storageclass: {}".format(
...
def get_pods():
    ...
    for pod in pods.items:
        ...
        if namespace == "all_ns":
            ...
            subtitle = "Status: {}, Namespace: {}, Restarts: {}".format(
                pod.status.phase,
            ...
            subtitle = "Status: {}, Restarts: {}".format(
                pod.status.phase, pod.status.container_statuses[0].restart_count
            )
        items.append(generate_dict(title, subtitle, arg, icon))
...
def get_deployments():
    ...
    for dp in deployments.items:
        ...
        icon = "deploy"
        if namespace == "all_ns":
            subtitle = "Ready: {}/{}, Namespace: {}".format(
                dp.status.ready_replicas, dp.status.replicas, dp.metadata.namespace
            )
            arg = "{}/{}".format(dp.metadata.name, dp.metadata.namespace)
        else:
            subtitle = "Ready: {}/{}".format(
                dp.status.ready_replicas, dp.status.replicas
            )
...
def get_statefulsets():
    ...
    for sts in statefulsets.items:
        ...
        if namespace == "all_ns":
            subtitle = "Ready: {}/{}, Namespace: {}".format(
                sts.status.ready_replicas, sts.status.replicas, sts.metadata.namespace
            )
...
def get_pvcs():
    ...
    for pvc in pvcs.items:
        ...
        if namespace == "all_ns":
            subtitle = (
                "Status: {}, Namespace: {}, Volume: {},  Storageclass: {}".format(
                    pvc.status.phase,
            ...
            subtitle = "Status: {}, Storageclass: {}, Volume: {}".format(
                pvc.status.phase, pvc.spec.storage_class_name, pvc.spec.volume_name
            )
...
def generate_dict(title, subtitle, arg, icon, uid=None):
...
def get_contexts():
    ...
    contexts, active_context = config.list_kube_config_contexts()
    items = [
        generate_dict("Active context", active_context["name"], "", "kubernetes", 0)
    ]
    for idx, context in enumerate(contexts, start=1):
        title = context["name"]
        subtitle = context["context"]["cluster"]
        arg = context["name"]
        icon = "kubernetes"
        uid = idx
        items.append(generate_dict(title, subtitle, arg, icon, uid))
